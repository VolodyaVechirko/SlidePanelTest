...
public class SwipeLayout extends ViewGroup {
     ...
     * <p>
     * Ignores {@link SwipeLayout#isSwipeEnabled()} and {@link SwipeLayout#isSwipeEnabled()}
     */
    ...
    private void layoutChildren(int left, int top, int right, int bottom) {
        ...
        int width, height, childLeft, childTop;

        centerView = getChildAt(0);
        childTop = parentTop;
        childLeft = centerView.getLeft();
        width = centerView.getMeasuredWidth();
        height = centerView.getMeasuredHeight();
        centerView.layout(childLeft, childTop, childLeft + width, childTop + height);

        rightView = getChildAt(1);
        childTop = parentTop;
        childLeft = centerView.getRight();
        width = rightView.getMeasuredWidth();
        height = rightView.getMeasuredHeight();
        rightView.layout(childLeft, childTop, childLeft + width, childTop + height);
    }
    ...
    private final ViewDragHelper.Callback dragCallback = new ViewDragHelper.Callback() {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            Log.d("ViewDragHelper", "tryCaptureView " + child.toString());
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            Log.d("ViewDragHelper", "clampViewPositionHorizontal " + child.toString() + " left " + left + " dx " + dx);
            if (dx > 0) {
        ...
        public int getViewHorizontalDragRange(View child) {
            Log.d("ViewDragHelper", "getViewHorizontalDragRange " + child.toString());
            return getWidth();
        }

        @Override
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            Log.d("ViewDragHelper", "onViewReleased " + releasedChild.toString() + " xvel " + xvel + " yvel " + yvel);
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            Log.d("ViewDragHelper", "onViewPositionChanged " + changedView.toString() + " left " + left + " top " + top + " dx " + dx + " dy " + dy);
    ...
    private void startScrollAnimation(View view, int targetX, boolean moveToClamp, boolean toRight) {
        if (dragHelper.settleCapturedViewAt(targetX, view.getTop())) {
            postOnAnimation(new SettleRunnable(view, moveToClamp, toRight));
        } else {
...
//
//                    if (((isLeftToRight && !isSwipeEnabled) || (!isLeftToRight && !isSwipeEnabled))
//                            &&
//                            getOffset() == 0) {
//
//                        return defaultResult;
